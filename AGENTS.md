...
| `plugins/workbench/` | `workbench` | supported | **Requirements-first delivery and grounded research.** A compact `.work/` ledger, collaborative ideation, externally sourced `.research/` evidence, adaptive execution across one or several epics, and deterministic knowledge indexing. Its `setup` skill converts older workflows into one clean state. Workbench and agile-workflow schemas are mutually exclusive within one project. |
