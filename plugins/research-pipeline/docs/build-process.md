...
**Syntactic vs semantic — and a known gap between them.** `rp:gate-citations` (and the inline `/citation-lint` the producers run) is the **syntactic** half: it mechanically proves every `[handle]{N}` resolves to a real attestation under `.research/attestation/` with present provenance — no fabricated, colliding, or unreachable citations. It checks the chain *resolves*; it does not read the attestation's passages, so it cannot judge whether the cited passage actually *supports* the claim.

The research **evaluators** (`/research` Phase 5; the `/deep-research` / `/research-program` Evaluator) are the **plausibility** half — but note their boundary honestly: they run in **isolated context (the brief + seed only, not the attestations)** by design, to avoid inheriting the author's framing. So they catch fabrication-smell, internally-unsupported or uncited claims, contradictions, and coverage gaps — **but they do NOT verify a cited passage supports its claim either**, because they never see the passages. **Passage-level source-support verification is a known gap** in this pipeline: it would require an adversarial-reader pass that receives the attestations *with full context* (ARD separates the isolated evaluator from a context-rich adversarial reader; we currently run only the former). Until that's added, treat "clean lint + clean evaluator" as "the chain resolves and nothing looks fabricated" — **not** as "every claim is verified against its source." Never overstate it.
