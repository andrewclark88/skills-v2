...
**Three complementary checks on research output — each does what the others can't.** Grounding is verified at three levels; none subsumes another, so run all three:

1. **Lint — syntactic (chain resolves).** `rp:gate-citations` (and the inline `/citation-lint` producers run) mechanically proves every `[handle]{N}` resolves to a real attestation with present provenance — no fabricated, colliding, or unreachable citations. It does **not** read the passages.
2. **Adversarial reader — passage-level support.** A fresh-context pass with **full access** (the brief + the attestation passages + the lint output) that walks each load-bearing claim back to its cited passage and verifies the passage actually *supports* the claim (`research-pipeline:adversarial-reader`, job catalog a–h from ARD CATALOGS §4). This is the check the other two structurally cannot do — it's the only one that reads the passages against the claims. `/research` runs it at Phase 4e, `/brief` at 4c, `/scout` after its lint, `/deep-research` + `/research-program` as a campaign/program-level read in Phase 9.
3. **Evaluator — isolated plausibility.** `/research` Phase 5 and the `/deep-research` / `/research-program` Evaluator run in **isolated context (brief + seed only)** by design (the FR.1 framing-bias fence), catching fabrication-smell, uncited/internally-unsupported claims, contradictions, and coverage gaps. The isolation is deliberate and is *why* it can't see passages — that's the adversarial reader's job, not a defect.

So: lint says the citation points somewhere real; the adversarial reader says the somewhere-real actually backs the claim; the evaluator (blind to the sources, on purpose) says nothing smells fabricated and coverage is complete. A clean run means all three passed — not just one.
...
| `rp:gate-citations` | Quality-checkpoint (research gate) | Citation-chain integrity of the `.research/` corpus — broken/colliding/unresolved `[handle]{N}`, thin attestations — as `gate_origin: citations` items. Syntactic; one of the three-check model (lint + adversarial-reader + isolated evaluator) |
