...
2. **A written brief** on disk at the specified path (main output; load-bearing for synthesis), citing every load-bearing claim `[handle]{N}`. The `[handle]` is the load-bearing anchor — the lint resolves the chain by handle and ignores `N`. Specialists do **not** share or append a corpus `INDEX.md` (that races); they cite with a **provisional local `N`** (so the citation is well-formed and lint-visible) and the orchestrator assigns authoritative `N`s post-merge (Phase 9). Brief frontmatter carries `provenance: agent-synthesis` (the brief is a synthesis artifact; only the attestation files are `source-direct`), plus a `## Disconfirming analysis` section (outcome of seeking disconfirming evidence) and a `## Contradictions` section when sources diverge (named-source positions side-by-side, no resolution-by-paraphrase).
...
4. **Assemble the authoritative corpus `INDEX.md`, normalize citations, then lint (post-merge reconciliation).** Parallel specialists wrote attestations to the one flat `.research/attestation/` dir (convergent by handle) and cited with provisional local `N`s — they did NOT maintain a shared `INDEX.md` (that races). Here, in the single parent context, do it once: (a) **assemble** `.research/reference/<corpus>/INDEX.md` by reading each attestation's frontmatter (`source_handle` + `source_url`/`source_path`) — one entry per unique handle, assigning the authoritative `N`; (b) **normalize** every `[handle]{N}` across the specialist briefs and `parent.md` to the assigned `N` (a per-handle find/replace — the lint ignores `N`, so this is for reader-facing bibliography integrity, not chain resolution); (c) run the `citation-lint` skill over the campaign directory against the shared attestation tier:
   ```
   /citation-lint .research/briefs/<seed-slug>/ --exit-code-on high
   ```
   This resolves every `[handle]{N}` in every specialist brief and `parent.md` against `.research/attestation/`. Because all specialists wrote to the **one flat attestation dir**, this is where a **`colliding-handle`** surfaces — two specialists that attested *different* sources under the *same* handle (high severity). Reconcile it: rename one source's handle (and its citations) so each handle names exactly one source. Also fix any `unresolved-handle` (a cited handle no specialist actually attested). Re-run until clean. The lint is **syntactic** (chain resolves).
5. **Adversarial source-support read across the campaign.** Dispatch one fresh sub-agent with **full context** — `[verbatim research-discipline bundle] + [verbatim ${CLAUDE_PLUGIN_ROOT}/skills/adversarial-reader/SKILL.md body] + all specialist briefs + `parent.md` + every cited attestation + the Phase-9 lint output`. It walks each load-bearing `[handle]{N}` claim back to its attestation passage and verifies the passage *supports* the claim (jobs a–h), writing `verification-checklist.md` at the campaign root with per-claim verdicts + `APPROVED`/`NEEDS-REVISION`. On `NEEDS-REVISION`, revise the flagged claims (narrow/re-attest/drop), re-run step 4 + this read, **and re-run the Phase 8 Evaluator on the revised briefs** — its earlier report is now stale (the reader runs after Phase 8 in this flow, so any revision it forces invalidates the evaluation). This is the **passage-level** check — it closes the gap the syntactic lint and the isolated Evaluator (Phase 8) both leave open. The three compose: lint = chain resolves; adversarial reader = passage supports the claim; Evaluator = isolated fabrication-smell/coverage. Run all three.
6. Write `campaign.md` (evaluator's report + campaign metadata)
7. Run `/knowledge-index` to regenerate the index from frontmatter (do NOT hand-edit `docs/knowledge-index.yaml`). Each brief must have conformant frontmatter (`description`, `type: brief`, `kind: research`, `slug`, `summary`, `key_findings`, `research_method: /deep-research`, `provenance: agent-synthesis`, `verification_status: attested` once the lint is clean — absent ⇒ `legacy-unattested`, `updated`)
8. **Chain mode (`--continue-from` passed):** update the parent campaign's `parent.md` by
