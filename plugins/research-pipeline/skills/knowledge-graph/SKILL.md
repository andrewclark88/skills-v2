...
- `--3d` — emit a **3D** view (`3d-force-graph`/three.js) instead of the 2D cytoscape graph, from the
  ...
  force-driven. Left-drag orbits, right-drag pans, scroll zooms (a toolbar button flips left-drag to
  pan). Includes search-to-focus, hover cards, a click→detail panel (in/out links), and edge-type
  toggles. Use for spatial exploration; the 2D view remains the surface for QA/audit (table + lenses).
