...
description: "The ARD anti-fabrication discipline bundle — the six load-bearing sections (source-bound citation, substrate test, no-footnote-fabrication, per-source attestation, contradiction-handling + seek-disconfirming, composed-claim) that bind any research-authoring context writing into this project's .research/ substrate. Body vendored verbatim from ARD kernel/discipline.md (v0.4.1). The /deep-research and /research-program orchestrators inline this bundle into every specialist Task dispatch so the discipline reaches spawned sub-contexts; it also auto-loads here for light-path inline authoring."
...
**The six numbered sections below are ARD's `kernel/discipline.md`, vendored verbatim** — this skill is the *wrapper* around them. They are **not re-narrated**; paraphrasing the discipline reintroduces the drift it fences (*ARD SPEC §4.6/§5*). Only this wrapper (above the sections) is deployment-specific; the sections are byte-identical to the pinned ARD release recorded in `plugins/research-pipeline/ard.json`.

**Propagation (this deployment's mechanism).** The `/deep-research` and `/research-program` skills inline this bundle, verbatim, into every authoring Task dispatch (the specialist role brief) — satisfying the *ARD SPEC §5* invariant ("the discipline must travel into every authoring sub-context"; the mechanism is a deployment choice, and ours is inline-into-dispatch because an auto-loading skill does NOT propagate into spawned sub-agents). On the light path (no *authoring* fan-out — `/research`, `/brief`, `/scout` author the brief and its attestations in the parent context) it auto-loads into the authoring context directly. Those skills may still spawn *investigation* sub-agents that gather raw material, but the parent attests from the actual fetched sources and is the bound authoring context. Either way the content arrives unaltered.

**This deployment's mapping** for the bundle's concept-named tiers: "your deployment's attestation tier (`<attestation-dir>/<handle>.md`)" is `.research/attestation/<handle>.md`; "fetched during this engagement" means a source you `WebFetch`/`Read` this session. Where §4 names `kernel/schema/attestation.schema.json`, that schema is vendored in this plugin at `plugins/research-pipeline/scripts/schema/attestation.schema.json` (per `ard.json`).
