...
Campaigns ran in parallel against the one shared `.research/attestation/` tier, so — as in `/deep-research` — they wrote attestations convergently (by handle) and cited with provisional local `N`s, but did NOT concurrently maintain a shared corpus `INDEX.md`. Here, once: (a) assemble the authoritative `INDEX.md` by reading each attestation's frontmatter (handle + url/path), one entry per unique handle; (b) normalize every `[handle]{N}` across campaign briefs and the super-parent to the assigned `N` (find/replace — the lint ignores `N`; this is for reader-facing bibliography integrity); (c) lint program-wide. The lint resolves the chain by **handle**, not `N` — so the program-wide pass catches cross-campaign `colliding-handle` (two campaigns, same handle, different sources), the real integrity check. Syntactic (the chain resolves).

**Then run the program-level adversarial source-support read.** Dispatch one fresh sub-agent with **full context** — `[verbatim research-discipline bundle] + [verbatim ${CLAUDE_PLUGIN_ROOT}/skills/adversarial-reader/SKILL.md body] + the super-parent + all campaign briefs + every cited attestation + the program-wide lint output`. It verifies, per load-bearing `[handle]{N}` claim, that the cited passage *supports* the claim (jobs a–h), writing `verification-checklist.md` at the program root with per-claim verdicts + `APPROVED`/`NEEDS-REVISION`; on `NEEDS-REVISION`, revise, re-run the lint + this read, **and re-run the Phase 8 Program Evaluator on the revised briefs** (its earlier report is staled by the revision). This closes the passage-level gap the lint and the isolated Program Evaluator (Phase 8) both leave open. The three compose: lint = chain resolves; adversarial reader = passage supports the claim; Program Evaluator = isolated fabrication-smell/coverage at program scale. Run all three.
