...
fetched: 2026-06-15
source_title: This paper introduces TheBotCompany, a framework for continuous multi-agent software development. Un…
...
1. This paper introduces TheBotCompany, a framework for continuous multi-agent software development.
2. Unlike single-session systems (ChatDev, SWE-agent), it sustains development over days, organizing work through a three-phase state machine managed by permanent manager agents who dynamically hire and retire worker agents based on current milestone demands.
3. Three-phase lifecycle (Strategy → Execution → Verification) governed by three permanent manager agents: Athena (Strategy/planning), Ares (Execution/implementation), Apollo (Verification/quality).
4. Strategy phase: Athena (planning manager) reviews project specifications, issue trackers, codebase state, and prior verification feedback; defines concrete next milestones with specific objectives; allocates a cycle budget.
5. Execution phase: "each cycle, Ares assesses progress and emits a schedule, which is an ordered list of workers and their tasks." Task ordering is determined by the manager agent's current-state assessment, not by static priority scores assigned at backlog-creation time.
6. Verification phase: an independent verification agent (Apollo) evaluates whether claimed milestones actually meet objectives; Apollo can reject incomplete work, generating structured feedback that triggers corrective cycles with halved budgets on subsequent attempts.
7. Feedback-driven anti-pattern accumulation: accumulated rejection feedback generated a documented list of "anti-patterns" — confirmed failing approaches that constrain future planning across subsequent milestones (a form of cross-session backlog hygiene).
8. SQLite-backed issue tracker modeled on GitHub Issues mediates all agent coordination; users file issues that managers consume at phase boundaries — "No ceremony-based synchronization points" (standups, grooming sessions) are required.
