...
1. Cargo defaults its optimization level to `3` for release builds, which optimizes the binary for **speed**. To instruct Cargo to optimize for minimal binary **size**, use the `z` optimization level"…
2. By default on Linux and macOS, symbol information is included in the compiled `.elf` file. This information is not needed to properly execute the binary." — §"`strip` Symbols from Binary
3. On macOS, the final stripped binary size is reduced to 51KB." — §"Optimize `libstd` with `build-std`
4. On macOS, the final stripped binary is reduced to 8KB." — §"Remove `core::fmt` with `#![no_main]` and Careful Usage of `libstd`
