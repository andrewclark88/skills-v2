...
fetched: 2026-06-15
source_title: An engineering blog post from Metabase describing their internally built Repro-Bot, a GitHub Actions…
...
1. An engineering blog post from Metabase describing their internally built Repro-Bot, a GitHub Actions-based agent that automates bug reproduction steps for GitHub issues.
2. Useful as a primary example of autonomous issue triage in a production codebase.
3. Human-gated activation by design: users must tag issues with `.Run Repro-Bot` to trigger the GitHub Action; this human-in-the-loop approach prevents injection attacks because "most issues come from our public GitHub issues."
4. The agent does not autonomously select which issues to work on; selection is entirely human-driven (via the `.Run Repro-Bot` tag) — a deliberate boundary keeping task selection human-controlled while execution is automated.
