...
fetched: 2026-06-15
source_title: SAGA introduces "program-level" scheduling for AI agent workloads, treating entire agent workflows a…
...
1. SAGA introduces "program-level" scheduling for AI agent workloads, treating entire agent workflows as first-class schedulable units rather than treating each LLM inference call independently.
2. The paper argues that current systems discard gigabytes of intermediate state (KV cache) between execution steps, inflating latency by 3–8×.
3. Agent Fair Share (AFS) prioritizes based on task urgency — assigning priority proportional to remaining work divided by deadline proximity, providing formal completion-time fairness guarantees via Lyapunov drift analysis (99.2% SLO attainment under multi-tenant interference).
4. The system achieves within 1.31× of Bélády's optimal offline caching policy and 1.64× task completion time reduction versus state-of-the-art baselines on production benchmarks.
