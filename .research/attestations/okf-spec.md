...
fetched: 2026-07-19
source_title: Open Knowledge Format (OKF) v0.1 Specification
...
1. | `index.md`   | Directory listing. See §6. | | `log.md`     | Update history. See §7. |
2. Index files contain no frontmatter. The body uses one or more sections, each grouping concepts under a heading.
3. Producers MAY generate `index.md` automatically; consumers MAY synthesize one on the fly when none is present.
4. `type` — A short string identifying the kind of concept. … Type values are **not** registered centrally. Producers SHOULD pick values that are descriptive and self-explanatory; consumers MUST toler…
5. [1] [BigQuery public dataset announcement](https://...) [2] [Internal data quality runbook](https://wiki.acme.internal/data/quality)
6. Citation links MAY be absolute URLs, bundle-relative paths, or paths into a `references/` subdirectory that mirrors external material as first-class OKF concepts.
7. `index.md` may appear in any directory and provides progressive disclosure by showing a human or agent what is available before individual documents are opened.
8. An OKF bundle is a self-contained hierarchical directory tree of Markdown knowledge documents, is the unit of distribution, and can be shipped as a Git repository, archive, or subdirectory.
9. An OKF concept ID is the path of the concept file within the bundle with the `.md` suffix removed.
10. A Markdown link between concepts asserts an untyped relationship whose kind comes from surrounding prose, and consumers must tolerate links whose targets do not exist.
11. OKF conformance requires parseable YAML frontmatter and a non-empty `type` on non-reserved Markdown files plus valid reserved files when present; consumption is otherwise intentionally permissive.
12. OKF v0.1 may be declared with `okf_version: "0.1"` in the bundle-root `index.md`; minor versions may add optional fields while major versions may change required fields or reserved filenames.
13. The optional `resource` frontmatter field is a URI that uniquely identifies the underlying asset.
14. OKF citations are numbered Markdown links, and the specification defines neither a `[handle]{N}`-style citation wire form nor a per-source attestation or fetch-state requirement.
15. Concept documents require `type`; recommended optional fields include `title`, `description`, `resource`, `tags`, and `timestamp`, and producers may include additional keys.
16. OKF is intentionally close to LLM wiki repositories, personal knowledge tools, and metadata-as-code approaches, but differs by specifying the small set of interoperability rules.
17. OKF v0.1 is an intentionally minimal, vendor-neutral format for a self-describing tree of Markdown knowledge files, and the repository includes three sample bundles as worked instances.
