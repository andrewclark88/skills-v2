...
fetched: 2026-06-15
source_title: An argument against categorical priority flags (P1/P2/P3, Blocker/High/Medium/Low) as queue-ordering…
...
1. An argument against categorical priority flags (P1/P2/P3, Blocker/High/Medium/Low) as queue-ordering signals, and a recommendation for priority-ordered lists where physical position is priority.
2. "One man's Blocker is another man's P4, it often turns out" — flags are subjective and inconsistently applied across individuals and teams.
3. The inflation mechanism: "When entering work, users know that lower priorities won't get done, so they default to higher tiers" — combined with the absence of any "one in, one out" constraint ("there's rarely anything in a priority flag system that limits the number of allowed P1s").
4. Recommended alternative: a priority-ordered list where items are physically ordered most-important first; "when a new item comes in (or an existing item is reordered), other items have to move to make room for it," forcing explicit trade-offs.
