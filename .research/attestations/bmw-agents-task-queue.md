...
fetched: 2026-06-15
source_title: BMW Agents is a multi-agent framework for task automation that uses a Planner Agent to decompose use…
...
1. BMW Agents is a multi-agent framework for task automation that uses a Planner Agent to decompose user requests into a dependency-aware Task Queue, which is then drained by specialized agents selected via semantic and mention matching.
2. The framework has no explicit priority scoring mechanism: tasks become eligible based on dependency satisfaction (readiness), and within the ready set ordering is capability-match driven rather than value-driven.
3. Tasks are held in the queue with dependency metadata and execute asynchronously once ready: "all tasks that are ready to be executed (all of their dependencies have been completed) are completed asynchronously."
4. The Planner Agent decomposes a user instruction into simpler tasks and identifies dependencies between them in a Directed Acyclic Graph (DAG); the DAG is the primary scheduling substrate.
5. Agent selection uses Semantic Matching (similarity between task descriptions and agent expertise), Mention Matching (@AgentName), and Iterative Matching (predefined sequences) — capability matching is the secondary selector after readiness.
