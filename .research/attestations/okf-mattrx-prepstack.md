...
fetched: 2026-07-20
source_title: Stop Chunking Documents: The Open Knowledge Format (OKF) for Enterprise AI (Mattrx)
...
1. replacing that with OKF + a Context Engine took our assistant's hallucination rate from 18% to 3% and our stale-answer rate from 11% to 1.5%
2. Knowledge base restructured from raw docs into ~11,000 OKF units (Markdown + metadata + relationships + APIs + schemas + business rules)
3. Governance and freshness controls, including `valid_until` and live API references, are enforced by the separate Context Engine layered on top of the OKF units.
4. The Context Engine uses hybrid, vector, and graph retrieval over the OKF units.
