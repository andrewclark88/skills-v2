---
id: priority-and-grooming-shapes-for-agentic-substrates
kind: research-brief
summary: For an agent-drained work substrate, priority should be a structural ordering signal (value field optional and conditional) and grooming should be a propose-not-prune capability mechanizing only what date/metadata supports.
updated: 2026-06-15
source_handles:
...
*Settled stance (2026-06-15): recommended shapes for two agile-derived capabilities on a
markdown-and-frontmatter substrate where an automated agent drains ready items by
dependency-readiness then FIFO. A research-time stance, not a design — it constrains the
design that follows without pre-empting its trade-offs.*
...
grooming/hygiene capability. This brief states the recommended *shape* for each. It is a
...
machine-sortable: a computed numeric score (WSJF/CD3 [wsjf-blackswanfarming]{3}, RICE
[rice-intercom]{1}), which decays and is gameable, and a ranked 1-N position, which is
inflation-proof but human-maintained [priority-inflation-agileforall]{1}
...
[story-points-agility-at-scale]{1}.

Independently, real autonomous agent systems order the ready set by structural graph properties
(critical-path depth / blocking potential [ai-metropolis-priority-queue]{3}, deadline urgency),
...
- **If a value field is added, prefer the shape with the least failure surface.** The evidence
  ...
  *filter*. *(Whether a filter-only tag suffices, or true ordering is needed, is a design
  trade-off this brief deliberately leaves open — it turns on how the signal is consumed.)*
- **Rejected as a primary ordering mechanism: an uncapped categorical priority flag**
  ...
  [priority-flags-agilefixer]{1}. If a categorical tag is adopted at all, it should be scoped as
...
three independent literatures: scoring frameworks build in human re-evaluation
[rice-intercom]{3}; a large maintainer survey wants AI as "a second pair of eyes" that does
"not intervene unless asked" [github-models-maintainer-survey]{2}, with approve-before-acting
tooling [dosu-issue-triage]{3}; and even autonomous execution systems retain human selection at
the trust boundary [metabase-reprobot-triage]{4}.
...
- **Mechanize only what date/metadata arithmetic supports, without claiming more.** Item age,
  ...
  dry-run, graduated exemptions, and a "mark, never auto-close" mode) [actions-stale-github]{1}.
  Duplicate/near-duplicate detection is *partially* mechanizable via embedding similarity with
  human confirmation [genai-backlog-grooming-empirical]{1} [dosu-issue-triage]{2}. Supersession
...
- **The disposition default for dead items inherits from the substrate's terminal-tier
  ...
  [actions-stale-github]{1} vs. archival-with-memory
  [backlog-antipatterns-age-of-product]{2} — bridged by a mark-without-close mode
  [actions-stale-github]{4}. A grooming design should not re-decide this; it should route
  DONE/SUPERSEDED through whatever terminal-tier retention the substrate already defines.
- **Cadence is configurable, not assumed-continuous.** Agent economics make continuous hygiene
  ...
  [augmentcode-ai-backlog-grooming]{6}. Leave cadence (continuous vs. wave/batch) to the
  deployment.
...
- **Survive as lessons, re-expressed structurally:** dependency ordering (already mechanized as
  ...
  work at scale [devcommunity-agents-backlog-coffee]{4}.

*(The ceremony-vs-lesson verdicts are synthesis inference over a disclosed-thin agentic
literature. They are well-corroborated where the priority and grooming facets overlap, but are
reasoning, not settled empirical finding.)*

## Disconfirming evidence

The priority and grooming *classical* groundings are solid (canonical and well-corroborated
secondary sources). The *agentic-translation* layer is thin and partly inferential:
peer-reviewed comparative studies of agent-queue vs. human-team backlog practice essentially do
not exist yet; the freshest claims rest on practitioner accounts and single-system evaluations.
The recommendations above are therefore stated as *shapes to prefer and traps to avoid*, not as
settled empirical certainties — and both capabilities are recommended as **optional,
inert-when-absent** precisely so a deployment that disagrees is unaffected. The clearest
disconfirmation considered and rejected was the possibility that an uncapped categorical
priority flag could work — the priority-inflation literature [priority-inflation-agileforall]{2}
[priority-flags-agilefixer]{1} forecloses that without an enforced scarcity constraint.
